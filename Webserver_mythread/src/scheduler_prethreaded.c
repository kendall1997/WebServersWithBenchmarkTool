...
void scheduler_prethreaded(int slot, struct Queue *requests, int poolSize)
{
    ...
    if (prethreaded_started == FALSE)
    {
        ...
        threadPool = (mythread_t *)calloc(poolSize, sizeof(mythread_t));
        ...
        for (threadCounter = 0; threadCounter < poolSize; ++threadCounter)
        {
            ...
            // create the thread
            mythread_create(&threadPool[threadCounter], NULL, prethreaded_task, (void *)&threadCounter);

            // Let it run in the background
            mythread_detach(threadPool[threadCounter]);
        }
    }
    ...
    if (prethreaded_working_count < poolSize)
    { // yeap there is
        ...
        for (threadCounter = 0; threadCounter < prethreaded_poolSize; ++threadCounter)
        {
            ...
            if (prethreaded_slots[threadCounter] == -1)
            { // -1 means available
...
void prethreaded_after_work(int threadId)
{
    ...
    mythread_mymutex_lock(&lockWrite);

    // check if there is more work to do
    if (getSize(prethreaded_request) == 0)
    { // Enjoy free time ;)
    ...
    mythread_mymutex_unlock(&lockWrite);
}
